...
class Wallet:
    ...
    def set_hotkey(self, keypair: Keypair, *, encrypt: bool = False,
                overwrite: bool = False, password: Optional[str] = None) -> None:
        self._hotkey = keypair
        self.hotkey_file.set_keypair(keypair, encrypt=encrypt, overwrite=overwrite, password=password)
                
    def set_coldkey(self, keypair: Keypair, *, encrypt: bool = True,
                overwrite: bool = False, password: Optional[str] = None) -> None:
        self._coldkey = keypair
        self.coldkey_file.set_keypair(keypair, encrypt=encrypt, overwrite=overwrite, password=password)
    ...
    def import_coldkey_from_mnemonic(self, mnemonic: str, *, password: Optional[str] = None,
        ...
        self.set_coldkey(kp, encrypt=bool(password), overwrite=overwrite, password=password)
        self.set_coldkeypub(kp, overwrite=overwrite)
        return self

    def import_hotkey_from_mnemonic(self, mnemonic: str, *, password: Optional[str] = None,
                                    overwrite: bool = False) -> "Wallet":
        kp = Keypair.create_from_mnemonic(mnemonic)
        self.set_hotkey(kp, encrypt=bool(password), overwrite=overwrite, password=password)
        return self
